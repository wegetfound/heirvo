// Standalone single-file HTML export for a library disc — browser fallback.
//
// This is the SOURCE OF TRUTH for the export's visual design. The Rust side
// (`src-tauri/src/library/html_export.rs`) MUST mirror this template
// byte-for-byte aside from line-ordering of escapes. When you tweak the
// design, update both.
//
// The produced .html file is fully self-contained: inlined CSS + ~30 lines
// of vanilla JS for the search box. Fonts load from Google on first online
// view, with system-serif/sans fallbacks.

import type { Disc, GradientId } from "./types";
...
function gradientFor(id: string): string {
  return (GRADIENTS as Record<string, string>)[id] ?? GRADIENTS.wedding;
}
...
export function renderDiscHtmlBrowser(disc: Disc): string {
  const grad = gradientFor(disc.gradient);
  const title = esc(disc.title);
  const date = esc(disc.date);
  const filmedBy = disc.filmedBy ? esc(disc.filmedBy) : "";
  const location = disc.location ? esc(disc.location) : "";
  const about = disc.about ? esc(disc.about) : "";
  const source = esc(disc.source);
  const duration = esc(disc.durationFormatted);
  const phrases = disc.phrasesIndexed.toLocaleString();
  const scenesCount = disc.scenes.length;
  const peopleCount = disc.people.length;
  const exportedOn = todayIso();

  const byline = [filmedBy && `Filmed by ${filmedBy}`, location]
    .filter(Boolean)
    .join(" · ");

  const topicsHtml = disc.topics
    .map(
      (t) =>
        `<span class="topic"><span class="topic-label">${esc(t.label)}</span><span class="topic-count">${t.count}</span></span>`,
    )
    .join("");

  const scenesHtml = disc.scenes
    .map(
      (s, i) => `
      <li class="scene">
        ...
        <div class="scene-body">
          <div class="scene-head">
            ...
            <span class="scene-ts">${esc(s.time)}</span>
          </div>
          ${s.description ? `<p class="scene-desc">${esc(s.description)}</p>` : ""}
        </div>
      </li>`,
    )
    .join("");

  const transcriptHtml = disc.transcript
    .map((line) => {
      const speakerHtml =
        !line.isStageDirection && line.speaker
          ? `<span class="speaker">${esc(line.speaker)}</span>`
          : "";
      const textClass = line.isStageDirection ? "line stage" : "line";
      const escapedText = esc(line.text);
      const dataText = escapedText.toLowerCase();
      return `<article data-text="${dataText}">
  <span class="ts">${esc(line.time)}</span>
  ${speakerHtml}
  <p class="${textClass}">${escapedText}</p>
</article>`;
    })
    .join("\n");
...
<body>
  <header class="hero">
    <div class="container hero-inner">
      <span class="eyebrow">Heirvo Archive · ${esc(source)}</span>
      <h1 class="disc-title">${title}</h1>
      ${byline ? `<p class="byline">${byline}</p>` : ""}
      ${date ? `<p class="byline">${date}</p>` : ""}
    </div>
  </header>
  ...
  <div class="container">
    <section class="meta">
      ...
      <div class="meta-cell">
        ...
        <div class="meta-value">${esc(source)}</div>
      </div>
      <div class="meta-cell">
      ...
      <div class="meta-cell">
        ...
        <div class="meta-value">${scenesCount} · ${peopleCount}</div>
      </div>
    </section>

    ${about ? `<section class="about"><h2 class="sect">About this memory</h2><p>${about}</p></section>` : ""}

    ${
      disc.topics.length > 0
        ? `<section><h2 class="sect">Topics</h2><div class="topics">${topicsHtml}</div></section>`
        : ""
    }

    ${
      disc.scenes.length > 0
        ? `<section><h2 class="sect">Scenes</h2><ol class="scenes">${scenesHtml}</ol></section>`
        : ""
    }
...
(function () {
  ...
  // Cache original text per article so re-typing restores cleanly.
