...
2. **Recovery loop + photo gallery — needs a disc.** Verifying the recovery→library bridge (`promote_session_to_library` → `disc_added` event → "Getting your video ready…" → playback) and the photo gallery (`disc_photos` rows + vault JPEGs + grid/lightbox) requires recovering a **real disc**. NOTE: a real optical drive (**TSSTcorp CDDVDW SH-224DB**) is now connected and the app shows "Drive ready" / "Slide a disc in" — so this is unblocked the moment a damaged DVD / photo CD is inserted. The code is compile-clean and the player half is proven via the bunny.
