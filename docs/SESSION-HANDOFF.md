...
1. **PCD decode — command-validated (`0edd2f7`); only a real Kodak disc's multi-res `[2]` remains.** Validated against a *synthetic* `.pcd` (generated via the installed magick: `magick rose: test.pcd`). **This caught a real bug:** `pcd_to_jpeg` used IM6-style `magick convert "<src>[N]" "<dst>"`, which current IM7 portable builds reject (`convert` parsed as a filename → "no decode delegate"). Fixed to IM7-native `magick "<src>[N]" "<dst>"` — CLI-proven to produce a valid JPEG. `pcd_to_jpeg` is a verbatim wrapper of that exact command (no AppHandle), so the magick logic is proven; only Tauri's generic IPC plumbing for `convert_special_image` (which has no UI caller yet — the recovery bridge in promote.rs is the real consumer) and the resolution-index behavior on a *genuine* multi-resolution Kodak PCD (`[2]`=768×512 on real discs; gave 384×256 on the single-layer synthetic) remain to confirm with a real Photo CD.
